...
Baryon can visualize an audio file, a microphone, a line input, or loopback audio from another app.

For live microphone input, choose the profile that matches the material:

| Source                                | Recommended profile | Why it helps                                                       |
| ------------------------------------- | ------------------- | ------------------------------------------------------------------ |
| Voice, singing, lead instrument       | **Voice**           | Tracks a dominant pitch and treats harmonics as supporting detail. |
| Water, wind, room tone, field audio   | **Ambient**         | Allows several sources to contribute at once.                      |
| Music playback or routed system audio | **Ambient**         | Avoids forcing mixed material into a single vocal driver.          |
...
For subtle natural recordings, start lower on **Reactivity**, **Density**, and **Opacity**, then raise **Node Threshold** only if the field feels too diffuse.

## Save a repeatable setup

Use **Presets** to save named snapshots of your controls. This is the quickest way to keep separate looks for water, wind, voice, or a particular track.
