  ...
  <Step title="Connect an audio source">
    ...
    - **Your microphone** — start the mic, allow access, and choose a profile:
      **Voice** for a lead voice or instrument, **Ambient** for music, a full
      mix, or a live room.
    - **System audio** — route another app's output through a loopback device
      (BlackHole on macOS, VB-Cable on Windows, PipeWire/PulseAudio on Linux) and
      select it as the input.

    **Checkpoint:** within a second or two, the orb starts moving with the sound.

    {/* TODO: swap the SVG placeholder for a real 4–6s muted, looping clip of
        audio coming alive into a field. Easiest: overwrite
        docs/images/quickstart/first-render.svg with a small looping GIF and
        point src at it. For an MP4 instead, replace the <img> with:
        <video autoPlay loop muted playsInline
          src="/images/quickstart/first-render.mp4" /> */}
    <Frame caption="The moment it clicks: a sound becomes a field.">
      <img
        src="/images/quickstart/first-render.svg"
        alt="Baryon rendering live audio as a volumetric cymatic field"
      />
    </Frame>
  </Step>
