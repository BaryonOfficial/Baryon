...
The Baryon GUI (top-right corner) lets you shape the visualizer in real time. Hover over any control in the panel to see its tooltip. This document covers the shared visual controls and marks controls that appear only in the web app or Listener Mode.
...
| Rotate the idle logo independently | Set **Logo Rotation** to `Manual`, then adjust **Logo Rotation Speed** in **Logo**.                 |
...
| Label          | Description                                                                                                                                                                                                       |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Profile**    | `Auto` adapts raymarch steps toward 60 FPS, `Custom` uses Target FPS, and `Max Quality` leaves raymarch steps ungoverned. Version 1.2.0 defaults to `Max Quality`; use `Auto` when you want adaptive performance. |
...
This control sets the opaque background used by the web app and Listener Mode.

| Label            | Description                           |
| ---------------- | ------------------------------------- |
| **Output Color** | Background fill color for the output. |

Performer does not expose this control. Display, Syphon, and Spout always
receive an opaque image over black; see [Program
output](/public/desktop/program-output).
...
| **Material Density** | Scale the uniform optical tracer that reveals caustic irradiance. This changes scattering and absorption, not acoustic or caustic topology. |
...
| **Max Samples**      | Set the maximum analytic camera-ray sample budget. `Auto` and `Custom` performance profiles may use fewer samples to hold their target.     |

---

## Appearance

Controls for tracer color, focused-light transfer, and bloom.

| Label                | Description                                                                                                             |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **Color Mode**       | `Static` uses the chosen colors. `Spectral` colors promoted cymatic modes from the audio spectrum.                      |
| **Volume Color**     | Main color of the volume interior.                                                                                      |
| **Caustic Color**    | Color of focused laser irradiance revealed by the tracer.                                                               |
| **Spectral Chroma**  | Adjust spectral color vividness at constant luminance without blending in the configured static colors.                 |
| **Caustic Strength** | Camera-visible focused-light gain. It does not change acoustic topology, ray bending, or material extinction.           |
| **Laser Focus**      | Inverse finite-source angular spread. Higher values produce narrower caustic folds without changing the acoustic field. |
...
Controls for cymatic-render movement. These settings do not rotate the idle logo.

| Label                   | Description                                                                                                                      |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Rotation Mode**       | `Audio` rotates the cymatic render with music, `Manual` uses Manual Rotation, and `Off` keeps it still.                          |
...
| **Pattern Persistence** | Sets the cymatic geometry exposure from 0.05 to 2 seconds. Higher values retain recognizable structures longer but overlap more. |
...
| **Logo Size**           | Size of the 3D idle logo shown when no audio is playing.                 |
| **Logo Color**          | Color of the 3D idle logo shown when no audio is playing.                |
| **Logo Rotation**       | `Manual` rotates the idle logo independently; `Off` keeps it stationary. |
| **Logo Rotation Speed** | Idle-logo spin speed in Manual mode — negative values reverse direction. |
...
| **Suppress Playback Telemetry** | Pause per-frame devtools snapshots and console audit logs during playback. Rendering quality and frame cadence are unchanged.        |
