...
For a practical walkthrough, start with [First session guide](/public/guides/first-session). For the conceptual model behind these settings, see [Understanding the engine](/public/guides/understanding-the-engine). Desktop OSC, OSCQuery, and MIDI control-input setup lives in [Parameter Automation](/public/desktop/parameter-automation) under the Desktop track.
