...
import gpgpuParticlesShader from "../three/shaders/gpgpu/particles.glsl";
...
export function gpgpuSetup(
  ...
  for (let i = 0; i < baseGeometry2.count; i++) {
    ...
    baryonLogoTexture.image.data[i4 + 0] =
      baseGeometry2.geometry.attributes.position.array[i3 + 0];
    baryonLogoTexture.image.data[i4 + 1] =
      baseGeometry2.geometry.attributes.position.array[i3 + 1];
  ...
  gpgpu.audioDataVariable.material.uniforms.uRadius = new THREE.Uniform(
    parameters.radius
  );
  ...
  gpgpu.scalarFieldVariable.material.uniforms.uRadius = new THREE.Uniform(
    parameters.radius
  );
  gpgpu.scalarFieldVariable.material.uniforms.uBase = new THREE.Uniform(
  ...
  gpgpu.zeroPointsVariable.material.uniforms.uThreshold = new THREE.Uniform(
    parameters.threshold
  );
  gpgpu.zeroPointsVariable.material.uniforms.uRadius = new THREE.Uniform(
    parameters.radius
  );
  gpgpu.zeroPointsVariable.material.uniforms.uSurfaceThreshold =
    new THREE.Uniform(parameters.surfaceThreshold);
  gpgpu.zeroPointsVariable.material.uniforms.uSurfaceControl =
    new THREE.Uniform(true);
  gpgpu.zeroPointsVariable.material.uniforms.uAverageAmplitude =
    new THREE.Uniform(0.0);

  // Dependencies
  gpgpu.computation.setVariableDependencies(gpgpu.zeroPointsVariable, [
  ...
  gpgpu.particlesVariable.material.uniforms.uBaryon = new THREE.Uniform(
    baryonLogoTexture
  );
  ...
  gpgpu.particlesVariable.material.uniforms.uParticleMovementType =
    new THREE.Uniform(1);
  gpgpu.particlesVariable.material.uniforms.uRadius = new THREE.Uniform(
    parameters.radius
  );
