...
import { guiSetup } from '../utils/guiSetup';
import { gpgpuSetup, disposeGPGPUResources } from '../baryon/gpgpuSetup';
import { particlesSetup } from '../baryon/particlesSetup';
import { createTimeHandler } from '../utils/timeHandler';
import { loadModelAndSetup } from '../utils/loadModelAndSetup';
import { setupLoaders } from './setupLoaders';
...
export default function useThreeScene(
  ...
  const statsRef = useRef(null); // Store reference to performance stats panel
  ...
  useEffect(() => {
    ...
    const { scene, sizes, camera, controls, renderer: _renderer } = setupScene(canvasRef);
    renderer = _renderer;
    ...
    const { effectComposer: composer, unrealBloomPass } = postProcessingSetup(renderer, scene, camera, sizes);
    ...
    (async () => {
      ...
      // Initialize stats panel (hidden by default)
      const stats = Stats();
      stats.showPanel(0);
      statsRef.current = stats;

      // Setup GUI controls and pass setShowStats to toggle stats visibility
      guiSetup(gui, unrealBloomPass, renderer, particles, gpgpu, debugObject, materialParameters, parameters, statsRef.current, setShowStats);
    })();
    ...
    const tick = () => {
      ...
      if (statsRef.current && showStats) statsRef.current.begin();
      ...
      gpgpu.particlesVariable.material.uniforms.uMicActive.value = audioObject.gumStream?.active;
      ...
      rotationMatrix.makeRotationY(-rotationTime.current * 0.5 * particles.material.uniforms.uRotation.value);
      ...
      // End performance stats measurement
      if (statsRef.current && showStats) statsRef.current.end();
    };
  ...
  }, [canvasRef, guiContainerRef, setIsPlaying, setIsAudioLoaded, setShowStats, showStats]);
}
