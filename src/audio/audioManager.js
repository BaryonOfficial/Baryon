...
// Audio object to store all audio-related state
...
// Audio loading and control functions
export function loadAudio(url) {
  return new Promise((resolve, reject) => {
    if (audioObject.sound.started) {
      audioObject.sound.stop();
    ...
    audioObject.audioLoader.load(
      ...
      (buffer) => {
        ...
        audioObject.isAudioLoaded = true;
        resolve();
      },
      undefined,
      (err) => {
...
export function playPauseAudio() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      } else if (!audioObject.sound.isPlaying && audioObject.isAudioLoaded) {
...
function audioAnalysis() {
  ...
  if (micIsActive && audioObject.micAnalyser) {
    const micAmplitude = audioObject.micAnalyser.getAverageFrequency();
    ...
    if (soundIsActive) {
      ...
      freqData = combineFrequencyData(freqData, micFreqData);
    } else {
      avgAmplitude = micAmplitude;
      freqData = micFreqData;
    }
  }

  return { avgAmplitude, freqData: Array.from(freqData) };
}
