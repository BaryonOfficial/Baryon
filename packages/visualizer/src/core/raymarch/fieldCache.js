...
function hashModeLayer(slots, activeCount) {
...
function resolveRebuildReason(previousDescriptor, nextDescriptor) {
...
export function createRaymarchFieldCache({
  ...
  return {
    ...
    dispatchSize: resolveDispatchSize(normalizedResolution),
    texture,
    active: false,
    ready: false,
...
export function buildRaymarchFieldCacheDescriptor({
  ...
  return {
    ...
    backboneHash: hashModeLayer(backboneSlots, backboneCount),
    detailHash: hashModeLayer(detailSlots, detailCount),
  };
}
...
export function shouldRebuildRaymarchFieldCache(fieldCache, descriptor) {
  ...
  const rebuildReason = resolveRebuildReason(
...
export function isRaymarchFieldCacheReadyForDescriptor(fieldCache, descriptor) {
  return Boolean(
    ...
    descriptorsEqual(fieldCache.activeDescriptor, descriptor),
  );
}
