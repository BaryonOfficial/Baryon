...
export function guiSetup(
  ...
  const resetDefaults = {
    reset: function () {
      [bloomFolder, colorFolder, granularControls, aesthetics].forEach(
        (folder) => folder.controllers.forEach((c) => c.reset())
      );
      // Re-apply side effects that onChange handlers would trigger
      renderer.setClearColor(debugObject.backgroundColor);
      particles.material.uniforms.uColor.value.set(materialParameters.color);
      particles.material.uniforms.uSurfaceColor.value.set(
        materialParameters.surfaceColor
      );
      gpgpu.zeroPointsVariable.material.uniforms.uThreshold.value =
        parameters.threshold;
      gpgpu.particlesVariable.material.uniforms.uThreshold.value =
