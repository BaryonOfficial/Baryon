...
export function guiSetup(
  ...
  granularControls
    ...
    .onChange(() => {
      gpgpu.zeroPointsVariable.material.uniforms.uThreshold.value =
        parameters.threshold;
      gpgpu.particlesVariable.material.uniforms.uThreshold.value =
        parameters.threshold;
    });
  ...
  aesthetics
    ...
    .onChange(function (value) {
      gpgpu.particlesVariable.material.uniforms.uParticleMovementType.value =
        value;
    });
  aesthetics.close();
    ...
    reset: function () {
      ...
      colorFolder.controllers.forEach((controller) => {
        ...
        if (controller.property === "surfaceColor") {
          ...
          particles.material.uniforms.uSurfaceColor.value.set(
            defaultSurfaceColor
          );
        }
      });
      ...
      granularControls.controllers.forEach((controller) => {
        if (
          ...
          controller._name === "FlowField Influence"
        )
          controller.setValue(1.0);
        if (
          controller.property === "value" &&
          controller._name === "FlowField Strength"
        )
          controller.setValue(3.6);
        if (
          controller.property === "value" &&
          controller._name === "FlowField Frequency"
        )
          controller.setValue(0.64);
        if (
          controller.property === "value" &&
          controller._name === "Particle Speed"
        )
          controller.setValue(32);
        if (
          controller.property === "value" &&
          controller._name === "Target Lerp Threshold"
        )
      ...
      aesthetics.controllers.forEach((controller) => {
        if (
          controller.property === "value" &&
          controller._name === "Particle Size"
        )
          controller.setValue(0.03);
        if (
          controller.property === "value" &&
          controller._name === "Rotation Speed"
        )
          controller.setValue(2.5);
        if (
          controller.property === "value" &&
          controller._name === "Surface Particles"
        )
          controller.setValue(true);
        if (
          controller.property === "value" &&
          controller._name === "Particle Movement Type"
        )
