...
describe("fast committed modal structural drive", () => {
  ...
  it("updates only selected forcing while preserving committed identities", () => {
    const structuralState = makeStructuralState();
    const excitationState = makeExcitationState();
    ...
    expect(exactDriveResult.targetEnergyByMode[1]).toBe(0);
    expect(
      response.entries.find((entry) => entry.modeKey === "1:1:1")
        ?.modalResponseDrive,
    ).toBeGreaterThan(0);
    expect(
      response.entries.find((entry) => entry.modeKey === "2:2:2")
        ?.modalResponseDrive,
    ).toBe(0);
    expect([
      ...structuralState.candidateForcingSlotsSource.slice(0, 3),
  ...
  it("replaces stale structural forcing metrics with the silent fast result", () => {
    ...
    const currentSignalAndForcingMetrics = [
      ...
      metrics.modalResponseRenderCapEnergy,
  ...
  it("maps exact drive evidence by committed mode instead of probe order", () => {
    ...
    expect(exactDriveResult.energyShareByMode[0]).toBe(0);
    expect(exactDriveResult.energyShareByMode[1]).toBeGreaterThan(0);
    expect(
      response.entries.find((entry) => entry.modeKey === "1:1:1")
        ?.modalResponseDrive,
    ).toBe(0);
